...
| Dashboard | React + Vite, nginx-served, dual-model detection view |
...
| Orchestration | Kubernetes (manifests + HorizontalPodAutoscalers for ingestion and ml, migrations as a Job) |
| Testing | pytest — 34 tests across all 3 Python services |
...
- **React + Vite dashboard**, nginx-served, with real navigation, a detail drawer per log, and a combined "IF + LSTM" indicator showing when both models agree — consuming the ingestion API directly rather than querying Postgres
...
- 34 automated tests across ingestion, ml (feature engineering + serving), and generator, run in CI on every push
- A lightweight tracked migration system (`docs/migrate.py` + `docs/migrations/`), also packaged to run as a Kubernetes Job — not ad-hoc manual schema changes
...
Six real bugs were found and fixed during development, each with a measured before/after or concrete verification — the kind of thing evaluation and testing are actually for:
...
6. **Stale container image cached under the `:latest` tag in Kubernetes.** After rebuilding the `ml` image locally to add migration support, a Kubernetes Job using `log-anomaly-dashboard-ml:latest` still ran the old image — confirmed by comparing the pod's actual image digest against the freshly-built one, which didn't match despite `latest` supposedly pointing at the new build. `imagePullPolicy: IfNotPresent` means Kubernetes won't re-pull just because a local rebuild happened. Fixed by forcing a deployment rollout restart; documented as a general gotcha (use an explicit non-`latest` tag, or force a rollout restart, whenever a rebuilt image doesn't seem to be picked up).
...
cp .env.example .env   # optional: fill in SLACK_WEBHOOK_URL to enable Slack alerting
...
- **This is a local-only deployment story.** The Kubernetes manifests are tested and working against a local cluster (Docker Desktop's built-in Kubernetes), sharing its local Docker image store. A real cloud deployment would need images pushed to a container registry, an Ingress controller, TLS, and a real domain — none of which are set up here, by deliberate choice (an always-on cloud deployment wasn't judged worth the recurring cost for a portfolio project; local + this documentation is considered sufficient proof).
...
- No tests for the training scripts (`train.py`, `train_lstm.py`) — they're mostly I/O orchestration (DB reads, model fitting, file writes), which is expensive and low-value to unit test; both the feature-engineering logic (`ml/features.py`) and the serving endpoints (`ml/predict_api.py`) are covered.
- No tests for the React frontend — validated through manual/visual testing rather than a JS test suite.
